 ...
 * @version     $Id: OpenidController.php 153 2009-05-10 21:20:21Z gnzsquall $
 */
...
class User_OpenidController extends Zend_Controller_Action {
	...
	protected function authenticate($username, $password) {
		...
		$authAdapter->setTableName ( $userModel->_name )->setIdentityColumn ( $userModel->getIdentityColumn () )->setCredentialColumn ( $userModel->getCredentialColumn () );
	...
	public function loginAction() {
		...
		if (! $this->getRequest ()->isPost () || ! $form->isValid ( $_POST )) {
			echo $form;
			return;
		} else if (! $consumer->login ( $form->openid_url->getValue () )) {
			echo "OpenID login failed.<br>";
		} else if (isset ( $_GET ['openid_mode'] )) {
			if ($_GET ['openid_mode'] == "cancel") {
				echo "CANCELED<br>\n";
			} else if ($this->_request->getParam('openid_mode') == 'id_res') {
