 ...
 *
 *   The actual table names should be: Gonium_acl_resources, Gonium_acl_access, Gonium_acl_roles, Gonium_acl_inheritance.
 *
...
class Gonium_ACL extends Zend_ACL
{
     ...
     *
     * - Set $mode to Gonium_ACL::ORPHAN_RESOURCE_ADD to add resources as "without parent".
     * - Set $mode to Gonium_ACL::ORPHAN_RESOURCE_IGNORE to ignore orphaned resources.
     * - Set $mode to Gonium_ACL::ORPHAN_RESOURCE_EXCEPTION to throw Exceptions.
     *
     * Default value of $mode is Gonium_ACL::ORPHAN_RESOURCE_EXCEPTION.
     * If $mode has a value different from the described ones, it will
     * treat like the default value.
     * 
     * @param Gonium_Model_Resource_Interface $resources
     * @param int $mode
     * @return void
     */
    public function loadResources(Gonium_Model_ACL_Resource_Interface $resources, $mode = self::ORPHAN_RESOURCE_EXCEPTION)
    {
    ...
    public function loadRoles($roles) //(Gonium_Model_Roles_Interface $roles)
    {
    ...
    public function loadAccess($access) //(Gonium_Model_Access_Interface $rules)
    {
        /// Now create all access rules
        //$access = $this->dbase->fetchAll( $this->dbase->select()->from( 'Gonium_acl_access', array( 'role_id','resource_id','privilege','allow' ) ) );
