...
class IndexController extends Zend_Controller_Action
{
    ...
    {
        $this->_view->headTitle($this->_lang->translate('Install'), 
        ...
        array(
            'module' => $this->_request->getModuleName(), 
                ...
                'action' => $this->_request->getActionName())//'action' => 'db-connection'
        ));
        
        if ($this->getRequest()->isPost() && $form->isValid($_POST) &&
         $this->tryConnection($form))
        {
            /* IMPORTAR SQL */
            $this->importSQL($form);
            /****************/
     ...
     //$this->_forward('writeConfig');
        ...
        } else
        {
            ...
            $form->system_baseAdminUrl->getValue() ? $form->system_baseAdminUrl->getValue() : $front->getBaseUrl() .
             '/admin/');
            
            $form->system_baseUrl->setValue(
            $form->system_baseUrl->getValue() ? $form->system_baseUrl->getValue() : $front->getBaseUrl() .
             '/');
            
            $form->dbConnection->dbhost->setValue(
            $form->dbConnection->dbhost->getValue() ? $form->dbConnection->dbhost->getValue() : 'localhost');
            $form->dbConnection->dbname->setValue(
            $form->dbConnection->dbname->getValue() ? $form->dbConnection->dbname->getValue() : strtolower(
            Gonium_Version::APP));
            $form->dbConnection->dbuser->setValue(
            $form->dbConnection->dbuser->getValue() ? $form->dbConnection->dbuser->getValue() : strtolower(
            Gonium_Version::APP));
    ...
    {
        if ($this->_request->isPost()) $config = $this->_request->getPost(
        'config');
    ...
    public function writeConfigAction ()
    {
        ...
        if ($config !== null && is_writeable($this->_etc) &&
         file_put_contents($destFile, $config))
        {
            $front = Zend_Controller_Front::getInstance();
            
            $this->_helper->viewRenderer->setScriptAction('index');
            $this->_view->messageSuccess = $this->_lang->translate(
            'Installation success');
            $this->_view->homeLinkVisible = true;
            
            $this->_view->homeLink = $this->_view->url(
            array('action' => 'index'));
    ...
    private function tryConnection (Gonium_Form_Prepared_SiteInstaller $dbForm)
    {
        ...
        try
        {
            $db = Zend_Db::factory($dbForm->dbConnection->dbadapter->getValue(), 
            array(
                'host' => $dbForm->dbConnection->dbhost->getValue(), 
                    ...
                    'dbname' => $dbForm->dbConnection->dbname->getValue())//'adapterNamespace' => 'MyProject_Db_Adapter
            );
    ...
    private function configure (Gonium_Form_Prepared_SiteInstaller $form)
    {
        ...
        // System
        $config->all->system = array('key' => $keygen);
        ...
        $config->all->resources->db = array(
            'adapter' => $form->dbConnection->getValue('dbadapter'), 
                ...
                'prefix' => $form->dbConnection->getValue(
                'dbprefix'), 
                'params' => array(
        ...
        $writer = new Gonium_Config_Writer_Ini(
        array('config' => $config, 'filename' => $this->_etc . '/config.ini'));
        
        if (is_writeable($this->_etc))
        {
            /** Config Form */
            $configForm = new Gonium_Form(
            array(
                'elements' => array(
                        'config' => array('hidden'), 
                            'submit' => array(
        ...
        } else
        {
            ...
            $this->_view->messageError = $this->_lang->translate(
            'Error: writing file');
            $this->_view->messageSuccess = sprintf(
            $this->_lang->translate(
            'Download config.ini file and upload to %1$s'), HOME_ROOT . '/etc/');
        }
