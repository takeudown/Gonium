...
class User_IndexController extends Zend_Controller_Action
{
    public function indexAction ()
    {
        ...
        // Title of Module
        $this->view->headTitle($lang->translate('User\'s Management'), 
        ...
        new Gonium_DataGrid_DataSource_Table($this->_helper->LoadModel('User')), 
        5);
        // Setting default datagrid values
        $grid->setDefaultSort('username')->setDefaultDir("asc");
        // Translate
        $grid->getPager()->translate($lang);
        
        $grid->addColumn('uid', 
        array('type' => 'text', 'header' => 'Identidad', 'width' => 10));
        
        $grid->addColumn('username', 
        array('type' => 'text', 'header' => $lang->translate('username')));
        ...
        array(
            'header' => $lang->translate('Delete'), 
                ...
                'actions' => array(
                    'url' => urldecode($url), 
                        'confirm' => _(
                        'Are you sure you want to delete $username ($uid)?'), 
                        'caption' => 'Delete', 
                        'image' => $this->view->baseUrl() .
    ...
    public function createAction ()
    {
        ...
        if ($this->getRequest()->isPost() && $form->isValid($_POST))
        {
            ...
            $userModel = $this->_helper->LoadModel(
            'User');
            $userModel->create($form->username->getValue(), 
