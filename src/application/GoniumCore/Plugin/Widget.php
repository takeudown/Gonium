...
class GoniumCore_Plugin_Widget extends Zend_Controller_Plugin_Abstract
{
	...
	*/
    ...
    Zend_Controller_Request_Abstract $request)
    {
        ...
        //{
        // Widget View
        $view = Zend_Registry::get(
        'GoniumCore_View');
        $dockView = clone $view;
        $dockView->setScriptPath(
        array(
            APP_ROOT . '/view/', 
        ...
        // Widget Layer
        $conf = new Zend_Config_Ini(HOME_ROOT . '/etc/widget.ini', 
        APP_ENV);
        ...
        foreach ($conf as $dockName => $dockContent)
        {
            if ($dockContent->dockLayout != null && $dockContent->widgets != null)
            {
                ...
                foreach ($dockContent->widgets as $widgetName => $widgetInfo)
                {
                    ...
                    if ($this->_layer->isEnabled() && ! is_string($widgetConf) &&
                     $widgetConf->class !== null)
                    {
                        ...
                        $widgetView->addScriptPath(
                        array(
                            APP_ROOT . 'GoniumCore/view/', 
                                APP_ROOT . '/Widget/' . $widgetDir .
                                 '/views/scripts/', 
                                HOME_ROOT . '/Widget/' . $widgetDir .
                                 '/views/scripts/', 
                                './'));
                        ...
                        {
                            $widgetView->registerHelper(
                            new Gonium_View_Helper_GlobalUrl(), 'GlobalUrl');
                            
                            $widgetView->getHelper('GlobalUrl')->setBaseUrl(
                            $widgetConf->globalUrl);
                            $widgetObject->setView($widgetView);
                        }
                        $dockObject->register($widgetObject, $widgetName, 
                        $widgetView);
                    }
    ...
    {
        if ($this->_layer->isEnabled()) $this->_layer->toResponse();
    }
}
